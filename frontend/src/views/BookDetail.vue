    ...
    <v-card class="rounded-2xl shadow-md bg-white p-6">
      ...
      <div class="grid md:grid-cols-2 gap-8 mb-10 px-6" v-if="book">
        ...
        <div class="flex flex-col justify-between">
          <div>
            ...
            <div
              class="flex items-center flex-wrap gap-3 text-gray-700 text-sm"
            >
      ...
      <div class="mt-10 px-6 py-6">
        ...
        <div class="mt-8">
          ...
          />
          <div>
            <v-btn class="mt-4 bg-black text-white" @click="submitReview"
              >Gửi đánh giá</v-btn
            >
          </div>
        </div>
    ...
    <v-dialog v-model="borrowDialog" max-width="500px">
      <v-card class="p-4">
        ...
        <p
          ...
          class="text-red-600 mt-2 text-center font-bold"
        >
          {{ borrowErrorMessage }}
        </p>
        <v-card-text>
          <!-- CẬP NHẬT Ở ĐÂY: Thêm min, max và v-model.number -->
          <v-text-field
          ...
          <p class="mt-2 text-sm text-gray-600">
            📅 Ngày trả dự kiến: <strong>{{ dueDate }}</strong>
          </p>
        </v-card-text>
        <v-card-actions>
          <v-spacer />
...
export default {
  ...
  methods: {
    // Hàm phụ trợ: Nếu người dùng cố tình nhập số > 5, tự động set lại về 5 (UX tốt hơn)
    checkQuantity() {
    ...
    borrowBook() {
      ...
      this.borrowForm.quantity = 1; // Reset về 1 khi mở
    ...
    async confirmBorrow() {
      ...
      // Vẫn giữ kiểm tra logic để bảo mật
